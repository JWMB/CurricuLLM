Given the structure of a textbook for the demographic "{{subject.demographic}}" (see below), generate a number of assignments/questions/problems for the student to solve.
...
The section you are to write questions for is “{{heading}}”, and is located here in the structure:
{{toc}}

Create sections, one for each difficulty level 1-3, where 1 is easy and 3 is difficult for the target group "{{subject.demographic}}". Create 5 to 10 questions for each difficulty level. About one-third can be questions with InputType=text, the rest should be single-choice or multiple-choice.
